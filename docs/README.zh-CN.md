...
| **用翻案句抬高普通改动**<br><br>“这次更新不是一次简单的界面调整，而是对工作效率的重新定义。搜索和导出从三级菜单移到了首页。” | **这次更新把搜索和导出从三级菜单移到了首页。**<br><br>**文案处置：** `rewrite` |
...
这些例子不会虚构功能或恢复路径。没有用户职责的文字直接删除，模板化翻案改成正面陈述，`42/31` 的事实保持不变，开发诊断移到合适位置，空状态只指向真实存在的操作。
