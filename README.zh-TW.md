...
Humanization 不會把每一句真實資訊潤飾後直接公開，而會先判斷這段文字在目前頁面是否有明確的使用者任務。

| 修改前 | 修改後 |
| :--- | :--- |
| 「這個公開網站沒有處理服務，不提供檔案上傳，不連接求職網站，也沒有開始任務的入口。」 | **不補任何公開文案。**<br><br>**文案處置：** `remove`<br>**產品流程：** `needs_product_decision`。這個頁面只提供閱讀資訊，還是應該讓訪客在這裡開始任務？如果分析功能屬於這個頁面，請先實作可用的入口與 CTA，再撰寫文案。 |

這些內部事實留在能力帳本中。原句無法協助訪客採取行動或做出判斷，因此直接移除。缺少入口是另一項產品決策，無法靠說明文字補齊流程。
