...
安装 `humanization` 后，用户可以显式指定 `zh-CN`、`zh-TW`、`en`、`ja`、`ko` 或 `es`，选择 `prose` 或 `web-microcopy`，并得到通用事实边界、对应语言规则和确定性硬规则检查。

## Acceptance

- Skill 目录名、front matter `name`、UI display name 和调用提示统一为 `humanization` / `Humanization`。
...
- `humanization/scripts/check_prose.py`
...
- Behavior: 完成 `humanization` 入口、六个 locale 档案、web microcopy 规则和显式校验 CLI；保留中文兼容检查入口。
