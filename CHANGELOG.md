...
- 新增任务内作者样本校准：只从用户明确指定的样本提取可观察、可重复的表达习惯，不建立永久作者画像，不迁移样本中的事实、经历或错误，也不覆盖 locale、事实、CTA、品牌词、无障碍和运行时结构约束。
- 新增跨语言通用的文案存在性门：先判断用户任务、决定、状态、恢复和必要披露，再决定 `keep`、`rewrite`、`move`、`remove` 或 `needs_product_decision`；内部能力清单不再默认流入公开文案。
