...
Humanization 不会把每句真实信息润色后直接发布，而是先判断这句话在当前页面有没有用户职责。

| 修改前 | 修改后 |
| :--- | :--- |
| “这个公开网站没有处理服务，不提供文件上传，不连接外部系统，也没有开始任务的入口。” | **不补任何公开文案。**<br><br>**文案处置：** `remove`<br>**产品流程：** `needs_product_decision`。这个页面只供阅读，还是应该让访客在这里开始任务？如果这里确实需要分析功能，应先实现真实入口和 CTA，再写文案。 |

这些内部事实留在能力账本中。原句无法帮助访客行动或判断，因此直接删除。缺少入口是另一项产品决策，不能靠解释性文案补齐流程。
