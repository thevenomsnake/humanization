...
<p align="center">
  ...
  <a href="#수정-전과-수정-후">수정 전과 수정 후</a> ·
  <a href="#규칙의-출처">출처</a> ·
...
<p align="center"><strong>제품에 들어갈 내용을 먼저 정한 뒤, 여섯 로캘에서 자연스럽게 씁니다.</strong></p>
...
공통 계약은 각 후보를 먼저 `keep`, `rewrite`, `move`, `remove` 중 하나로 분류합니다. 후보에서 빠진 동작, 상태, 복구 경로가 드러나면 해당 제품 흐름에 `needs_product_decision`도 함께 표시합니다. 내부 정보는 제품이 주장할 수 있는 범위를 정하고, 콘텐츠 판단은 공개 문구로 쓸 내용을 고릅니다. 로캘 프로필은 게시하기로 한 메시지를 각 언어에서 자연스럽게 표현하는 방식을 정합니다. 번역문의 문장 구조와 길이는 달라도 대상 독자, 행동, 기능 범위, 개인정보 약속, 승인된 용어는 같아야 합니다.

원자료가 부족하면 Skill은 한 가지에 초점을 맞춰 질문하거나, 출처를 확인하거나, 주장의 범위를 좁힙니다. 페이지에 지원되는 동작이나 복구 경로가 없으면 `needs_product_decision`을 반환해 제품 결정을 요청합니다. 자동 검사는 명확히 입증된 훼손을 실패로 처리합니다. 메시지의 필요성, 어조, 자연스러움은 문맥에 따라 사람이 검토합니다.

## 수정 전과 수정 후

Humanization은 사실이라는 이유만으로 모든 문장을 공개 문구로 다듬지 않습니다. 먼저 현재 화면에서 문구가 맡을 역할을 확인한 뒤 다시 쓸지, 옮길지, 공개 문구를 두지 않을지 판단합니다.
...
| **정형화된 대조로 변경을 과장한 경우**<br><br>“이번 업데이트는 단순한 반품 내역 검색 개선이 아니라 반품 업무의 새로운 기준을 제시합니다. 주문 번호나 구매자 이름으로 반품 내역을 찾을 수 있게 했습니다.” | **이번 업데이트에서는 반품 내역 검색을 개선해 주문 번호나 구매자 이름으로 찾을 수 있게 했습니다.**<br><br>**문구 처리:** `rewrite` |
...
어떤 예시도 기능이나 복구 경로를 새로 만들어 내지 않습니다. 사용자에게 필요 없는 문구는 삭제하고, 정형화된 대조는 구체적인 서술로 바꾸며, `42/31` 사실은 그대로 보존하고, 개발자 진단은 알맞은 위치로 옮기고, 빈 상태에서는 실제로 지원하는 동작만 안내합니다.
...
기존 문구를 다듬기 전에 현재 화면에 필요한 내용인지 점검할 수 있습니다.

```text
locale=zh-CN, format=web-microcopy, surface=public-page로 $humanization을 사용하세요. 각 기능 설명을 `keep`, `rewrite`, `move`, `remove` 중 하나로 분류하고, 제품 결정이 필요한 근본 문제도 표시하세요.
```
